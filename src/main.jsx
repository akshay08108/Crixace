...
function DetailPanel({ match, details, onClose }) {
  ...
  return <aside className="detail-panel">
    ...
    <div className="detail-tab-panel tab-panel" role="tabpanel" key={activeTab}>
      {activeTab === 'Scorecard' && <div className="batting scorecard-status"><p>LIVE SCORECARD</p><div><span>{match.state === 'live' ? 'Automatically refreshing' : 'Latest available result'} <small>{match.state === 'live' ? 'team totals every 5 minutes; player figures every 10 minutes' : 'from Sportmonks Cricket'}</small></span><b>{match.state === 'live' ? 'LIVE' : 'FINAL'}</b></div>{details?.batters?.map(batter => <div key={batter.name}><span>{batter.name} <small>not out</small></span><b>{batter.runs} <small>({batter.balls})</small></b></div>)}{details?.bowler && <div><span>{details.bowler.name} <small>over {details.bowler.currentOver}, ball {details.bowler.ballsInCurrentOver}</small></span><b>{details.bowler.overs} <small>overs</small></b></div>}{!details && <small>Detailed player figures are not available for this match yet.</small>}</div>}
      ...
      {activeTab === 'Info' && <div className="detail-info"><div><span>Venue / format</span><b>{match.competition}</b></div><div><span>Match status</span><b>{match.note}</b></div><div><span>Data source</span><b>Sportmonks Cricket API</b></div></div>}
    </div>
  </aside>;
}
...
function App() {
  ...
  return <div className="app">
    ...
    <main className="dashboard">
      {activeSection === 'Live scores' && <div className="tab-panel" key="live-scores-page">
        ...
        <div className="content-grid">
          <section className="scores">{featuredMatch ? <FeatureMatch match={featuredMatch} details={featuredDetails} onOpen={openMatch} onSave={() => setToast('Match saved to favourites')} /> : <div className="feed-empty"><span className="section-kicker">LIVE FEED</span><h2>Sportmonks reports no covered live matches</h2><p>The connection is working. Matches included in your Sportmonks subscription will appear automatically when their live feed starts.</p></div>}<div className="section-title"><h2>Matches</h2><button onClick={() => setFilter('All')}>View all <ChevronRight size={15} /></button></div><div className="match-list tab-panel" role="tabpanel" key={`${sport}-${filter}`}>{sport === 'cricket' && filter === 'Upcoming' && fixturesState.loading ? <DataLoading label="Loading upcoming matches" /> : visible.length ? visible.map((match, index) => <MatchCard key={`${match.id}-${index}`} match={match} onOpen={openMatch} />) : <p className="list-empty">{filter === 'All' ? 'Sportmonks returned no matches covered by this subscription.' : `Sportmonks returned no covered ${filter.toLowerCase()} matches.`}</p>}</div></section>
    ...
    <footer><span>© 2026 CrixAce</span><span>{apiConnected ? 'Live data powered by Sportmonks Cricket.' : 'Live cricket data is temporarily unavailable.'}</span></footer>
  </div>;
}
