...
function DetailPanel({ match, onClose }) {
  ...
  return <aside className="detail-panel">
    ...
    <div className="detail-tab-panel tab-panel" role="tabpanel" key={activeTab}>
      {activeTab === 'Scorecard' && <div className="batting scorecard-status"><p>LIVE SCORECARD</p><div><span>{match.state === 'live' ? 'Automatically refreshing' : 'Latest available result'} <small>{match.state === 'live' ? 'every 5 minutes while this tab is visible' : 'from Cricket Data'}</small></span><b>{match.state === 'live' ? 'LIVE' : 'FINAL'}</b></div><small>Detailed batter and bowler figures require scorecard access from the data provider.</small></div>}
    ...
    <main className="dashboard">
      ...
      <div className="content-grid">
        <section className="scores">
          ...
          <div className="match-list tab-panel" role="tabpanel" key={`${sport}-${filter}`}>{visible.map(match => <MatchCard key={match.id} match={match} onOpen={openMatch} />)}</div>
        </section>
